...
for (const file of files) {
  ...
  while (true) {
    try {
      await sql.begin(async (tx) => { await tx.unsafe(content); await tx`INSERT INTO migrations (name) VALUES (${file})`; });
      console.log('OK ' + file);
      break;
    } catch (e) {
      attempt++;
      const msg = String(e.message || e).slice(0, 100);
      if ((e.code === 'CONNECTION_CLOSED' || /connection|timeout|EOF/i.test(msg)) && attempt < 5) {
...
console.log('DONE ' + (await sql`select count(*)::int as n from information_schema.tables where table_schema='public'`)[0].n + ' tables');
await sql.end();
